...
class MyWindow(QMainWindow):
    ...
    def add_graphics_view(self):
        ...
        if self.horizontal_button.isChecked():
            # Add QGraphicsView in horizontal direction
            row = self.graphics_grid_start_row
            col = num_graphics_views % 2
            if num_graphics_views % 2 == 0 and num_graphics_views != 0:
                row += num_graphics_views // 2
        else:
            # Add QGraphicsView in vertical direction
            col = 0
            row = self.graphics_grid_start_row + num_graphics_views
        
        self.main_layout.addWidget(graphics_view, row, col, 1, 2 if self.horizontal_button.isChecked() else 1)
