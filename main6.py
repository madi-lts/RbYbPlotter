...
class MyWindow(QMainWindow):
    ...
    def add_graphics_view(self):
        ...
        if self.horizontal_button.isChecked():
            ...
            colspan = 3
            self.graphics_layout.addWidget(graphics_view, row, col, 1, colspan, alignment=Qt.AlignmentFlag.AlignCenter)
        else:
            row = self.graphics_grid_start_row + num_graphics_views
            col = 0
            rowspan = 3
            self.graphics_layout.addWidget(graphics_view, row, col, rowspan, 1, alignment=Qt.AlignmentFlag.AlignCenter)
