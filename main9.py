...
class MyWindow(QMainWindow):
    ...
    def add_graphics_view(self):
        ...
        colspan = int(self.col_span_edit.text()) + 1  if self.col_span_edit.text() else 1
        rowspan = int(self.row_span_edit.text()) + 1 if self.row_span_edit.text() else 1
        if self.horizontal_button.isChecked():
            row = self.graphics_grid_start_row
            col = num_graphics_views
            # Determine the colspan based on the user input or default to 1
            ...
            col = 0
            # Determine the rowspan based on the user input or default to 1
            self.graphics_layout.addWidget(graphics_view, row, col, rowspan, colspan)
