...
class MyWindow(QMainWindow):
    ...
    def add_graphics_view(self):
        ...
        if self.horizontal_button.isChecked():
            ...
            # Determine the colspan based on the number of columns in the previous row
            colspan = 1
            self.graphics_layout.addWidget(graphics_view, row, col, 1, colspan) # alignment=Qt.AlignmentFlag.AlignLeft)
        elif self.vertical_button.isChecked():
            row = self.graphics_grid_start_row + num_graphics_views
            col = 0
            colspan = num_graphics_views
            self.graphics_layout.addWidget(graphics_view, row, col, 1, colspan, alignment=Qt.AlignmentFlag.AlignTop)
